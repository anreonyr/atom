...
# 产物存在（cargo 不保证 bin 间构建顺序）。
# cargo 不跟踪 link.ld 变化（-Tlink.ld 经 rustflags 传入，无 build.rs rerun）——
# touch 源文件强制重编译重链接，避免改链接脚本后产出陈旧 ELF
cargo build --release --bin spawned
cp target/riscv64gc-unknown-none-elf/release/spawned spawned.elf
...
if command -v readelf >/dev/null 2>&1; then
  ...
  echo "--- spawned.elf:"
  LC_ALL=C readelf -h spawned.elf | grep -E 'Type:|Machine:|Entry point'
else
