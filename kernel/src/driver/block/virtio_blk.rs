...
#[inline]
unsafe fn read_reg(base: *const u8, offset: usize) -> u32 {
...
/// 调用方须保证 `base` 指向已映射的 virtio-mmio 区域。
#[inline]
unsafe fn write_reg(base: *mut u8, offset: usize, val: u32) {
...
fn submit(base: *mut u8, block_size: usize, ty: u32, block: u32, q: &mut VirtQueue) {
    ...
    unsafe {
        ...
        // 通知设备
        write_reg(base, REG_QUEUE_NOTIFY, 0);
    }
...
impl crate::hal::block::BlockDevice for VirtioBlk {
    ...
    fn ack_interrupt(&self) {
        ...
        unsafe {
            let status = read_reg(self.base, REG_INTERRUPT_STATUS);
            write_reg(self.base, REG_INTERRUPT_ACK, status);
        }
    }
}
...
impl Driver for VirtioBlkDriver {
    ...
    fn probe(&self, dev: &Device) -> Result<(), DriverError> {
        ...
        let magic = unsafe { read_reg(base, REG_MAGIC) };
        ...
        // SAFETY: 见上。
        unsafe {
            write_reg(base, REG_STATUS, 0); // 设备复位
            ...
            let features_hi = read_reg(base, REG_DEVICE_FEATURES);
            if features_hi & 1 == 0 {
                return Err(DriverError::Init(
            ...
            write_reg(base, REG_DRIVER_FEATURES, 1);
            // FEATURES_OK 校验
            write_reg(
                base,
                REG_STATUS,
        ...
        unsafe {
            ...
            let num_max = read_reg(base, REG_QUEUE_NUM_MAX);
            if (QSZ as u32) > num_max {
                return Err(DriverError::Init("virtio-blk: queue size exceeds num_max"));
            }
            write_reg(base, REG_QUEUE_NUM, QSZ as u32);
            let gpa = page_ptr as usize as u64;
            write_reg(base, REG_QUEUE_DESC_LOW, gpa as u32);
            write_reg(base, REG_QUEUE_DESC_HIGH, (gpa >> 32) as u32);
            let avail = gpa + AVAIL_OFF as u64;
            write_reg(base, REG_QUEUE_DRIVER_LOW, avail as u32);
            write_reg(base, REG_QUEUE_DRIVER_HIGH, (avail >> 32) as u32);
            let used = gpa + USED_OFF as u64;
            write_reg(base, REG_QUEUE_DEVICE_LOW, used as u32);
            write_reg(base, REG_QUEUE_DEVICE_HIGH, (used >> 32) as u32);
