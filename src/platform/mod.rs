// 平台模块 — 硬件探测、设备发现、FDT 解析
//
// 引导流程：
//   1. `platform::init(dtb_ptr)` — 解析 DTB，填充 PlatformConfig + 设备发现缓冲区
...
pub use config::{config, init, report_diag, PlatformConfig};
pub use discovery::for_each;
pub use dtb::{Dtb, Node};
