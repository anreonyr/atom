...
// 本层只承载服务集成，是 UART 驱动（driver/uart/）与消费方（console 设备
...
//     终端语义集中在本层
//   - `register()` 联动：console 设备选择层（device::register，一次构造
...
pub fn register<U: Uart + 'static>(uart: &'static U) {
    ...
    let name: &'static str = {
        let idx = crate::device::count();
        Box::leak(alloc::format!("uart{idx}").into_boxed_str())
    };
    let _ = crate::device::register(crate::device::ConsoleDevice::new(
        name,
        Some(writer),
        Some(input),
    ));
