...
//   1. 经 mprintln!（sbi 模块，M-mode SBI ecall）无锁直写——绕过 OUT_LOCK 与
//      console 设备表锁，避免持锁中崩溃时死锁
...
//
// 输出底座：sbi::mprintln!（M-mode 控制台直写，见 src/sbi/mod.rs）。
// 不依赖 log.rs 与 console 的 OUT_LOCK——panic 可能发生在持有任意锁
