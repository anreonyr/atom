...
// 认识一类 compatible（id_table），由 bus 匹配设备后调用 probe()。
...
pub enum DriverError {
    ...
    ///
    /// bus 的 `probe_all()` 会对返回此错误的设备自动重试，直到所有设备
...
/// 经角色目录（`serial`/`controller`）的 `DRIVERS` 常量汇总进 bus。
...
pub trait Driver: Send + Sync {
    ...
    /// # Deferred 语义
    ///
    /// 依赖尚未就绪时返回 [`DriverError::Deferred`]，bus 会延后到下一轮重试。
